...
export async function onSubscriptionUpdated(
	...
	try {
		...
		//@ts-ignore	
		const periodStart = subscriptionUpdated.current_period_start ? new Date(subscriptionUpdated.current_period_start * 1000) : new Date();
		//@ts-ignore
		const periodEnd = subscriptionUpdated.current_period_end ? new Date(subscriptionUpdated.current_period_end * 1000) : new Date();
